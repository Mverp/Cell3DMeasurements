...
{
	public final static String MANUALPOINTS = "Manual_Points", MEXICANHAT = "MexicanHat", DISTANCETRANSFORM = "DistanceTransform", WATERSHEDPONTS = "Watershed_Points";
	private final static String MAX = "Maximafinder";
	 ...
	 * @param aMethod
	 *            The method used to create the marker file
	 *
	 * @return A string with the additional name of the file
	 */
	static private String createAdditionOfFileName(final double aSigmaXY, final double aSigmaZ, final String aMethod)
	{
		...
		if (aMethod.contains(MEXICANHAT))
		{
	...
	{
		return aImage.getShortTitle() + MARKER_FILE_INFIX + aDetectionMethod;
	}
	...
	static private double[] dialogMexicanHatSettings()
	{
		...
		dia.addNumericField("Minimal size of a nucleus", 10, 1);
		dia.addNumericField("Maximal size of nucleus", 20, 1);
		dia.addNumericField("Stepsize of hat", 5, 0);
		dia.addCheckbox("Display LoG Kernel", false);
		dia.addCheckbox("Process per slice", false);
		dia.addCheckbox("Show intermediate images", false);

		dia.addMessage("Parameters for maxima filter");
		dia.addNumericField("Noise", 1, 2);
		dia.addNumericField("Minimum value", 0, 2);
		dia.addNumericField("RadiusXY", 0, 2, 5, "pixel");
	...
	static protected void mexicanHatMaster(final ImagePlus aOriginalImage, final String aTitlemethod, final File aOutputDirectory)
	{
		...
		for (double cellDiameter = min; cellDiameter <= max; cellDiameter = cellDiameter + step)
		{
			...
			sigmas.add(sigma);
			mexicanHatName = mexicanHatName + createAdditionOfFileName(sigmaXY, sigmaZ, MEXICANHAT);
		...
		final String maxFinderName = createAdditionOfFileName(xyRadius, minSigmaZ < 1 ? 1 : minSigmaZ, MAX);
	...
	{
		// Save marker file
		final File fileFiles = new File(aOutputDirectory.getPath() + File.separator + "Marker_Files");
		if (!fileFiles.exists())
		{
			if (fileFiles.mkdir())
			{
				IJ.log("   Directory has been created!");
			}
			else
			{
		...
		final String name = fileFiles.getPath() + markerfilename;
		aResults.save(name);
		IJ.log("   Marker file is save as: " + markerfilename + " in " + name);

		// Save marker image
		final File fileImages = new File(aOutputDirectory.getPath() + File.separator + "Marker_Images");
		...
		final String name1 = fileImages.getPath() + imageName;
